...
class BilibiliWatchTimeMiner:
    def __init__(self, config: MinerConfig, logger: logging.Logger | None = None) -> None:
        self.config = config
    ...
    async def _thread_loop(self, plan: SessionPlan, thread_index: int) -> None:
        ...
        try:
            ...
            self._logger.info("直播间 %s 连接 #%s 已启动", plan.room_id, plan.session_no)
    ...
    def _thread_entry(self, plan: SessionPlan, thread_index: int) -> None:
        try:
            ...
            self._logger.exception("直播间连接异常退出: %s", exc)
    ...
    def run(self) -> None:
        ...
        if self.config.task_ids:
            ...
            self._logger.info("任务追踪未开启（未设置任务 ID）")

        if self._notifier.enabled:
            self._logger.info("通知推送已开启（%s 个地址）", len(self.config.notify_urls))
        elif self.config.notify_urls:
            self._logger.warning("通知地址已配置但推送服务不可用")
        ...
        try:
            ...
            if alive_threads:
                ...
                self._logger.warning("停止未完成，仍有线程未退出: %s", preview)
            else:
                self._logger.info("所有连接已停止")
